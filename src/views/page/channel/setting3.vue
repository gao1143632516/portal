<template>
  <div class="app-container">
    ...
    <div style="display: inline-block;width: 100px;">
      <span>结算单价(元)</span>
      <el-input v-model.trim="settlement_price" auto-complete="off" size="mini" :disabled="true"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲量</span>
      <el-input v-model.trim="chuanshanjia_num" auto-complete="off" size="mini" :disabled="true"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲实时</span>
      <el-input v-model.trim="chuanshanjia_num_infact" auto-complete="off" size="mini" :disabled="true"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>实时差异</span>
      <el-input v-model.trim="chuanshanjia_num_infact_difference" auto-complete="off" size="mini" :disabled="true"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲留存(%)</span>
 ...
 <!--    <el-popover
  ...
  <el-table :data="shiMoHistoryList" :row-style="{height:'5px'}"
    ...
    <div style="display: inline-block;width: 100px;">
      <el-button size="mini">石墨量3</el-button>
    ...
    <div style="display: inline-block;width: 100px;">
      ...
      <div style="display: inline-block;width: 100px;">
      <span>毛利率(%)</span>
      <el-input v-model.trim="gross_margin" auto-complete="off" :disabled="true" size="mini"></el-input>
    </div>

     <div style="display: inline-block;width: 100px;">
      <span>留存系数(%)</span>
      <el-input v-model.trim="retained_coefficient" auto-complete="off" :disabled="true"  size="mini"></el-input>
    </div>
    ...
    <el-table-column cell-class-name="cell-class"
     ...
     <!--  <el-table-column label="哎呦留存" width="100" cell-class-name="cell-class">
      ...
      <el-table-column label="结算数" width="100" cell-class-name="cell-class">
        ...
        <el-table-column label="截图留存" :disabled="true" width="100" cell-class-name="cell-class">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getChannelAppData(app_id, activation_date) {
      getChannelAppData(app_id, activation_date).then(response => {
        if (response.code == 0) {
          ...
          this.shimo_num = typeof response.data.shimo_num3 == "undefined" ? 0 : response.data.shimo_num3;
    ...
    find() {
      ...
      this.getChannelAppUserData(app_id, activation_date,3);
          ...
          datalist.forEach(item => {
               if(item.settlement_rate == 0 && item.settlement_num==0 && item.income==0 && item.disburse==0 && item.update_ts==0){//没有结算过
                  ...
                  // console.log(yuqishouru);
                   ...
                   if(item.settle_is_zero == 1){//設置石墨三时 把石墨一比例为零的设置默认为0
                       item.settlement_rate = 0;
                   } 
