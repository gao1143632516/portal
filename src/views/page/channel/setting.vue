<template>
  <div class="app-container">
    ...
    <div style="display: inline-block;width: 100px;">
      <span>结算单价(元)</span>
      <el-input v-model.trim="settlement_price" auto-complete="off" size="mini"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲量</span>
      <el-input v-model.trim="chuanshanjia_num" auto-complete="off" size="mini"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲实时</span>
      <el-input v-model.trim="chuanshanjia_num_infact" auto-complete="off" size="mini"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>实时差异</span>
      <el-input v-model.trim="chuanshanjia_num_infact_difference" auto-complete="off" size="mini"></el-input>
    </div>
    <div style="display: inline-block;width: 100px;">
      <span>穿山甲留存(%)</span>
      <el-input v-model.trim="chuanshanjia_keep" @blur.prevent="change_retained_coefficient()" auto-complete="off" size="mini"></el-input>
    </div>
 <!--    <el-popover
  ref="popover4"
  placement="right"
  width="300"
  trigger="click">
  <el-table :data="shiMoHistoryList" :row-style="{height:'5px'}"
    :cell-style="{padding:'0px'}">
    <el-table-column width="150" property="create_ts" label="日期"></el-table-column>
    <el-table-column width="100" property="shimo_num" label="数量"></el-table-column>
  </el-table>
</el-popover> -->
    <div style="display: inline-block;width: 100px;">
      <el-button size="mini">石墨量1</el-button>
    ...
    <div style="display: inline-block;width: 100px;">
      ...
      <div style="display: inline-block;width: 100px;">
      <span>毛利率(%)</span>
      <el-input v-model.trim="gross_margin" auto-complete="off" size="mini"></el-input>
    </div>

     <div style="display: inline-block;width: 100px;">
      <span>留存系数(%)</span>
      <el-input v-model.trim="retained_coefficient" auto-complete="off"  size="mini"></el-input>
    </div>
...
<!--     <div style="margin: 20px;">
   ...
   <div class="total" style="margin: 20px">
    ...
    <el-table :data="dataList" v-loading="listLoading" :row-class-name="tableRowClassName"
     style="font-size: 10px" header-row-class-name="tableHead" height="800"
     stripe
   ...
   >
    ...
    <el-table-column cell-class-name="cell-class"
     ...
     <el-table-column label="渠道" width="100" cell-class-name="cell-class">
      ...
      <el-table-column label="有效新增" width="100" cell-class-name="cell-class">
        <template slot-scope="scope"> 
          <span style="margin-left: 10px">{{ scope.row.efficacious_num }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-input auto-complete="off" type="number" size="mini" v-model="scope.row.outer_settlement_price" @input="recount(scope.$index,scope.row)"></el-input>
        </template>
      ...
      <el-table-column label="结算比例%" width="100" cell-class-name="cell-class">
        <template slot-scope="scope">
          <el-input auto-complete="off" size="mini"  v-model="scope.row.settlement_rate" @input="recountRate(scope.$index,scope.row)"></el-input>
         
        </template>
      </el-table-column>
      <el-table-column label="结算数" width="80" cell-class-name="cell-class">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
          ...
          <el-input auto-complete="off" size="mini"  v-model="scope.row.chuanshanjia_keep"></el-input>
        </template>
      </el-table-column>
  <!--     <el-table-column label="channel_id" width="100">
      ...
      <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column label="操作" width="150" cell-class-name="cell-class" >
        <template slot-scope="scope">
          <el-button size="mini" @click="toSave(scope.$index, scope.row)">出图</el-button>
          <el-button size="mini" v-if="scope.row.is_out==0" type="danger" @click="findPicDetail(scope.$index, scope.row)">明细</el-button>
          <el-button size="mini" v-else type="" @click="findPicDetail(scope.$index, scope.row)">明细</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getChannelAppData(app_id, activation_date) {
      getChannelAppData(app_id, activation_date).then(response => {
        if (response.code == 0) {
          ...
          this.shimo_num = typeof response.data.shimo_num == "undefined" ? 0 : response.data.shimo_num;
    ...
    find() {
      ...
      this.getChannelAppUserData(app_id, activation_date,1);
      ...
      getChannelAppUserData(app_id, activation_date,type).then(response => {
        ...
        if (response.code == 0) {
          datalist.forEach(item => {
               if(item.settlement_rate == 0 && item.settlement_num==0 && item.income==0 && item.disburse==0 && item.update_ts==0){//没有结算过
